...
- From [[https://www.youtube.com/watch?v=Ng0NE4lDP2U][Fidonet]] we try and vibe off of the energy of [[https://en.wikipedia.org/wiki/Tom_Jennings][Tom Jennings]], but
  actually keep that anarchist spirit alive by avoiding all centralized
  authority.
...
good persistent storage guarantees. The ideas of `ssb` and `dat` are
...
When communicating with other devices, you will generally announce an
...
devices together and lists current signaling servers and dropboxes.)/
...
2. publish frequented signaling servers
...
A group of /signaling servers/ are published with the =synod=, so if you
want to send a message to that user or be able to sync up their latest
=head= you have a place to start.

Some /signaling servers/ can optionally accept incoming drop
...
non-followed identities that you can choose to read, acknowledge, reply
...
super old entries whose =identities= aren't being shared anymore.
...
Some signalling servers provide a /dropbox/ service, which allows blobs
...
propagation, which all clients are required to provide, though they
...
| data lease   | Storing of blobs with some guarantees                  |
| voice        | Voice calling                                          |
| video        | Video calling                                          |
...
If two clients are connected over =Websockets= to the same signalling
...
if the client is operating on a battery, or over a metered cellular
connection).
wit
...
3. All head requests with a synod identity with a final head must be
   marked invalid.
