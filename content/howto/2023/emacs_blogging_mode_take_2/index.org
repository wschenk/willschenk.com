...
#+begin_src elisp :tangle ~/.emacs.d/local/blog.el
  ...
  (defun blog-mode-make-fragment-draft ()
    "Create a new fragment with date-based naming"
    (interactive)
    (let* (
           (year (format-time-string "%Y"))
           (filename (format-time-string "%Y%m%d-%H%M%S"))
           (rootpath (concat blog-mode-base-dir "/fragments/" year "/" filename))
           (path (concat rootpath ".org"))
           )
      (set-buffer (find-file path))
      (insert "#+title: Fragment " (format-time-string "%Y-%m-%d %H:%M") "\n")
